...
export function bindClients(root: HTMLElement): void {
  const search = root.querySelector<HTMLInputElement>("#client-search");
  const statusFilter = root.querySelector<HTMLSelectElement>("#status-filter");
  const groupFilter = root.querySelector<HTMLSelectElement>("#group-filter");
  const selectAll = root.querySelector<HTMLInputElement>("#select-all");
  const rows = root.querySelectorAll<HTMLTableRowElement>(".client-row");
  const countEl = root.querySelector<HTMLElement>("#client-count");
  ...
  selectAll?.addEventListener("change", () => {
    root.querySelectorAll<HTMLInputElement>(".client-check").forEach((cb) => {
      cb.checked = selectAll.checked;
    });
  });

  root.querySelectorAll<HTMLButtonElement>(".ping-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      showToast(`Ping sent to ${btn.dataset["id"]}`, "info");
    });
  });

  root.querySelectorAll<HTMLButtonElement>(".uninstall-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      showToast(`Uninstall queued for ${btn.dataset["id"]}`, "warn");
    });
  });

  root.querySelector<HTMLButtonElement>("#bulk-screenshot")?.addEventListener("click", () => {
    showToast("Mass screenshot command sent to all online clients", "success");
  });

  root.querySelector<HTMLButtonElement>("#bulk-update")?.addEventListener("click", () => {
    showToast("Update command queued for all clients", "info");
  });

  root.querySelector<HTMLButtonElement>("#bulk-shutdown")?.addEventListener("click", () => {
    const checked = root.querySelectorAll<HTMLInputElement>(".client-check:checked");
    if (checked.length === 0) {
