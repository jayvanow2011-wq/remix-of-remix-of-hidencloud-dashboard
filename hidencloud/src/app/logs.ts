...
export function bindLogs(root: HTMLElement): void {
  const listEl = root.querySelector<HTMLElement>("#log-list")!;
  const searchEl = root.querySelector<HTMLInputElement>("#log-search");
  const typeFilter = root.querySelector<HTMLSelectElement>("#log-type-filter");
  let logs: LogEntry[] = [];
  ...
  root.querySelector<HTMLButtonElement>("#refresh-logs")?.addEventListener("click", load);
  root.querySelector<HTMLButtonElement>("#clear-logs")?.addEventListener("click", async () => {
    await api.clearLogs();
    logs = [];
    render();
  });
  root.querySelector<HTMLButtonElement>("#export-logs")?.addEventListener("click", () => {
