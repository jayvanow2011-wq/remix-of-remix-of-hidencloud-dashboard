...
export function renderDashboard(data: StatsResponse): string {
  ...
  const countryRows = Object.entries(countryMap)
    .map(
      ([country, count]) =>
        `<div class="summary-row"><span>${country}</span><span>${count} client${count > 1 ? "s" : ""}</span></div>`,
    )
    .join("");
  ...
  const groupRows = Object.entries(groupMap)
    .map(
      ([group, count]) =>
        `<div class="summary-row"><span>${group}</span><span>${count}</span></div>`,
    )
    .join("");
